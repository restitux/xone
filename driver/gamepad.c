...
static const guid_t gip_gamepad_guid_middle_button = GUID_INIT(0xecddd2fe, 0xd387, 0x4294,
...
static bool gip_gamepad_is_eliteseries2(struct gip_client *client)
{
    ...
    return (hw->vendor == GIP_GP_VID_MICROSOFT && hw->product == GIP_GP_PID_ELITE2);
}
...
static int gip_gamepad_op_input(struct gip_client *client, void *data, u32 len)
{
    ...
    input_report_abs(dev, ABS_HAT0X, !!(buttons & GIP_GP_BTN_DPAD_R) - !!(buttons & GIP_GP_BTN_DPAD_L));
    input_report_abs(dev, ABS_HAT0Y, !!(buttons & GIP_GP_BTN_DPAD_D) - !!(buttons & GIP_GP_BTN_DPAD_U));
    input_sync(dev);
...
static int gip_gamepad_probe(struct gip_client *client)
{
    ...
    gamepad->eliteseries2 = gip_gamepad_is_eliteseries2(gamepad->client);

    gamepad->led.rgb = gamepad->eliteseries2;
    err = gip_init_led(&gamepad->led, client);
    if (err)
	return err;

    if (gamepad->eliteseries2) {
	err = gip_init_profile(&gamepad->profile, client, gip_gamepad_driver.gip_root);
	if (err)
	    return err;
    }
